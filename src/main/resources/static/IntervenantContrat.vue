...
// charges les données du contrat
// liste de contrats
function chargeContrat() {
  const idIntervenant = route.params.num_SS // ID de l'intervenant que l'on souhaite afficher
  // Appel à l'API pour avoir la liste des contrats
  doAjaxRequest(`${BACKEND}/api/intervenants/${idIntervenant}/contrats`)
      .then((json) => {
        data.listeContrat = json._embedded.contrats;
        console.log(data.listeContrat);
      })
      .catch((error) => alert(error.message));
}

// A l'affichage du composant
// on charge les intervenants et les contrats
onMounted(() => {
  chargeIntervenant();
  chargeContrat();
});
    ...
    <div>
      <div class="table-container">
        <table class="tableintervenant">
          ...
          <tr class="body" v-for="contrats in data.listeContrat" :key="contrats.id">
            <!-- on met les informations de l'intervenant -->
            <td>{{data.intervenant.nom}}</td>
            <td>{{data.intervenant.prenom}}</td>
            <!-- on met sous format date -->
            <td>{{ formatDate(parseISODate(contrats.debut)) }}</td>
            <!-- on met sous format date -->
            <td>{{ formatDate(parseISODate(contrats.fin))}}</td>
            <td>{{ contrats.duree_en_jours }}</td>
            <td>{{ contrats.motif }}</td>
          </tr>
        </table>
      </div>
    </div>

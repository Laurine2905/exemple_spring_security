...
// on met les informations dans l'entité contrat
  fetch("http://localhost:8989/api/arrets", fetchOptions)
      .then((response) => {
        ...
        <ul id="bandeau">
          <li>
            <div class="menu">
              ...
              <div> Ajouter un arrêt : {{data.intervenant.nom}} {{data.intervenant.prenom}} </div>
            </div>
          </li>
        </ul>
      </nav>
    ...
    <div class="table-container">
      <table class="tableajout">
        ...
        <tr>
          ...
          <th>
            <button class="add"
                    @click="handlerAdd(date_debut, date_fin, nb_jours, objet )">
              Ajouter
            </button>
          </th>
        </tr>

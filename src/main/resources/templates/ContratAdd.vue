...
function handlerAdd(date_debut, date_fin, nb_jours, motif) {
  ...
  const fetchOptions = {
    ...
    body: JSON.stringify({
      debut: date_debut,
      fin: date_fin,
      duree_en_jours: nb_jours,
      motif: motif,
      // route.params.num_SS = le num_SS de la personne qui avait la fiche personnel
...
// on met les informations dans l'entité contrat
  fetch("http://localhost:8989/api/contrats", fetchOptions)
      .then((response) => {
        return response.json();
      })
      .then((dataJSON) => {
        console.log(dataJSON);
        window.alert("Contrat ajouté"); // afficher fenêtre pop-up de succès
        // vider les champs
...
// on charge l'intervenant pour avoir son num ss de deja rentré dans l'ajout d'un contrat
        ...
        <ul id="bandeau">
          <li>
            <div class="menu">
              ...
              <div> Ajouter un contrat : {{data.intervenant.nom}} {{data.intervenant.prenom}} </div>
            </div>
          </li>
        </ul>
      </nav>
    ...
    <div class="table-container">
      <table class="tableajout">
        ...
        <tr>
          ...
          <th>
            <input id="motif" v-model="motif" type="text" placeholder="Motif">
          </th>
        </tr>
        <tr>
          <th class="colonne">* Les champs suivants sont obligatoires</th>
          <th>
            <!-- lorsqu'on click sur le bouton on ajoute un contrat à la personne et on vide les champs-->
            <button class="add"
                    @click="handlerAdd(date_debut, date_fin, nb_jours, motif )">
              Ajouter
            </button>
          </th>
        </tr>
